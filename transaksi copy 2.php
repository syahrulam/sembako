    ...
    $(document).ready(function () {
        ...
        function calculateTotalPrice(itemContainer) {
            ...
            if (!isNaN(hargaSatuan) && !isNaN(jumlah)) {
                ...
                itemContainer.find(".total_harga").val(formatNumber(totalHarga)); // Format angka dengan ribuan
            }
        }
        ...
        function updateTotalHarga() {
            ...
            $('#itemTable tbody tr').each(function () {
                let totalItem = parseFloat(unformatNumber($(this).find('td:eq(4)').text()));
                if (!isNaN(totalItem)) {
                    totalHarga += totalItem;
                }
            });

            $('#total_harus_dibayar').val(formatNumber(totalHarga)); // Tampilkan total harga dengan format ribuan
            updatePembayaran();
        }
            ...
            if (!isNaN(bayar) && bayar > 0) {
                if (tipePembayaran === 'Cash') {
                    ...
                    if (bayar >= totalHarga) {
                        ...
                        $('#kembalian').val(formatNumber(parseInt(kembalian))); // Tampilkan kembalian dengan format ribuan
                    } else {
                    ...
                    if (bayar < totalHarga) {
                        ...
                        $('#kurangan').val(formatNumber(parseInt(kurangan))); // Tampilkan kurangan dengan format ribuan
        ...
        // Update format ribuan pada input Bayar (Rp.)
        $('#uang_diterima').on('input', function() {
            let input = $(this).val();
            let unformatted = unformatNumber(input);

            if (!isNaN(unformatted) && unformatted !== "") {
                let formatted = formatNumber(unformatted);
                $(this).val(formatted); // Tampilkan dengan format ribuan
            } else {
                $(this).val(''); // Kosongkan jika bukan angka
            }

            updatePembayaran(); // Update pembayaran setelah format berubah
        });

        // Update format ribuan pada Harga Satuan (Rp.) di tabel item
        ...
        function calculateTotalPrice(itemContainer) {
            var hargaSatuan = parseFloat(unformatNumber(itemContainer.find(".harga_satuan").val())); // Hapus format ribuan untuk perhitungan
            var jumlah = parseInt(itemContainer.find(".jumlah").val());

            if (!isNaN(hargaSatuan) && !isNaN(jumlah)) {
                var totalHarga = hargaSatuan * jumlah;
                itemContainer.find(".total_harga").val(formatNumber(totalHarga)); // Tampilkan total harga dengan format ribuan
            }
        }
